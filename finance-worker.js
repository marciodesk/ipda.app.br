...
<body>
  ...
  <main class="content"><div class="content-inner">
    <div class="module-view" id="view-dashboard">
      ...
      <section class="market"><div class="ticker" id="market-ticker">MERCADO FINANCEIRO IPDA \u2022 FECHAMENTO MENSAL \u2022 ATUALIZA\xC7\xC3O EM TEMPO REAL</div><div class="market-grid">
        ...
        <div class="quote"><small>Saldo consolidado</small><strong id="metric-balance">R$ 0,00</strong></div><div class="quote"><small>D\xEDzimos informados</small><strong id="metric-tithe">R$ 0,00</strong></div>
    ...
    <div class="module-view" id="view-churches" hidden>
      <div class="module-title"><h2>Igrejas</h2><p>Cadastre e organize a estrutura das igrejas.</p></div>
    <section class="card">
    ...
    <section class="card">
      ...
      <form id="financial-form">
        ...
        <div class="grid financial">
          ...
          <div class="money"><label for="tithe">D\xEDzimo</label><span>R$</span><input id="tithe" name="tithe" inputmode="decimal" placeholder="0,00"><small class="hint">Valor informativo; n\xE3o entra no c\xE1lculo do saldo.</small></div>
        </div>
  ...
  <script>
    ...
    async function render(){reports.innerHTML='<div class="empty">Carregando relat\xF3rios\u2026</div>';try{const data=await readReports();reportsCache=data;if(!data.length){reports.innerHTML='<div class="empty">Nenhum relat\xF3rio salvo.</div>';return}reports.innerHTML='<table><thead><tr><th>A\xE7\xF5es</th><th>Data</th><th>Igreja</th><th>TOTVS</th><th>Respons\xE1vel</th><th>Membros</th><th>Entradas</th><th>Sa\xEDdas</th><th>Saldo</th><th>D\xEDzimo</th><th>Documentos</th><th>Relat\xF3rio</th></tr></thead><tbody>'+data.map(r=>'<tr><td><button class="table-action" type="button" data-edit-report="'+escapeHtml(r.id)+'">Editar</button><button class="table-action danger" type="button" data-delete-report="'+escapeHtml(r.id)+'">Excluir</button></td><td>'+escapeHtml(r.report_date)+'</td><td>'+escapeHtml(r.church)+'</td><td>'+escapeHtml(r.totvs_code||'\u2014')+'</td><td>'+escapeHtml(r.responsible)+'</td><td>'+r.members+'</td><td class="amount">R$ '+currency(r.income)+'</td><td class="amount">R$ '+currency(r.expenses)+'</td><td class="amount '+(r.balance<0?'negative':'positive')+'">R$ '+currency(r.balance)+'</td><td class="amount">R$ '+currency(r.tithe||0)+'</td><td>'+(r.documents.length?r.documents.map(d=>'<a href="/financeiro/documentos/'+encodeURIComponent(d.id)+'" target="_blank">'+escapeHtml(d.filename)+'</a>').join('<br>'):'\u2014')+'</td><td><a class="button-link" href="/financeiro/relatorios/'+encodeURIComponent(r.id)+'/imprimir" target="_blank">Exportar PDF</a></td></tr>').join('')+'</tbody></table>'}catch(error){reports.innerHTML='<div class="empty">'+escapeHtml(error.message)+'</div>'}}
    ...
    [income,expenses,tithe].forEach(el=>el.addEventListener('blur',()=>{el.value=currency(number(el.value));updateBalance()}));
...
async function dashboardData(month, env) {
  ...
  const reports = (await env.DB.prepare(`SELECT church_id, church, income, expenses, balance, tithe, report_date, created_at
  ...
  for (const report of reports) {
    ...
    const current = totalsByChurch.get(report.church_id) || { income: 0, expenses: 0, balance: 0, tithe: 0, last_report: report.report_date };
  ...
  const rows = churches.map((church) => {
    ...
    return { ...church, status: totals ? "Fechado" : "Pendente", income: totals?.income || 0, expenses: totals?.expenses || 0, balance: totals?.balance || 0, tithe: totals?.tithe || 0, last_report: totals?.last_report || null };
  });
  const closed = rows.filter((church) => church.status === "Fechado").length;
  const summary = rows.reduce((all, church) => ({ ...all, income: all.income + church.income, expenses: all.expenses + church.expenses, balance: all.balance + church.balance, tithe: all.tithe + church.tithe }), { income: 0, expenses: 0, balance: 0, tithe: 0 });
...
function dashboardCsv(data) {
  ...
  const headers = ["M\xEAs de refer\xEAncia", "Igreja", "TOTVS", "Classifica\xE7\xE3o", "Cidade", "Estado", "Situa\xE7\xE3o", "Entradas", "Sa\xEDdas", "Saldo", "D\xEDzimo informativo", "\xDAltimo envio"];
  ...
  const summary = [data.month, "TOTAL GERAL", "", "", "", "", `${data.summary.closed} fechadas / ${data.summary.pending} pendentes`, decimal(data.summary.income), decimal(data.summary.expenses), decimal(data.summary.balance), decimal(data.summary.tithe), ""];
...
async function saveReport(request, env, reportId = '') {
  ...
  try {
    ...
    const values = [String(report.report_date || ""), church, responsible, Math.max(0, Number(report.members) || 0), Number(report.income) || 0, Number(report.expenses) || 0, (Number(report.income) || 0) - (Number(report.expenses) || 0), Number(report.tithe) || 0, churchId || null];
    const statements = [existing ? env.DB.prepare(`UPDATE financial_reports SET report_date = ?, church = ?, responsible = ?, members = ?, income = ?, expenses = ?, balance = ?, tithe = ?, church_id = ? WHERE id = ?`).bind(...values, reportId) : env.DB.prepare(`INSERT INTO financial_reports (report_date, church, responsible, members, income, expenses, balance, tithe, church_id, id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(...values, reportId)];
