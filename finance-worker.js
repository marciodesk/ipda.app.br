...
<body>
  ...
  <main class="content"><div class="content-inner">
    <div class="module-view" id="view-dashboard">
      ...
      <section class="market"><div class="ticker" id="market-ticker">MERCADO FINANCEIRO IPDA \u2022 FECHAMENTO MENSAL \u2022 ATUALIZA\xC7\xC3O EM TEMPO REAL</div><div class="market-grid">
        ...
        <div class="quote"><small>Saldo consolidado</small><strong id="metric-balance">R$ 0,00</strong></div><div class="quote"><small>D\xEDzimos informados</small><strong id="metric-tithe">R$ 0,00</strong></div><div class="quote"><small>Valores transferidos</small><strong id="metric-transferred">R$ 0,00</strong></div>
    ...
    <section class="card">
      ...
      <form id="financial-form">
        ...
        <div class="grid financial">
          ...
          <div class="money"><label for="tithe">D\xEDzimo</label><span>R$</span><input id="tithe" name="tithe" inputmode="decimal" placeholder="0,00"><small class="hint">Valor informativo; n\xE3o entra no c\xE1lculo do saldo.</small></div><div class="money"><label for="transferred">Transferido</label><span>R$</span><input id="transferred" name="transferred" inputmode="decimal" placeholder="0,00"><small class="hint">Valor informativo; n\xE3o entra no c\xE1lculo do saldo.</small></div>
        </div>
  ...
  <script>
    ...
    async function render(){reports.innerHTML='<div class="empty">Carregando relat\xF3rios\u2026</div>';try{const data=await readReports();reportsCache=data;if(!data.length){reports.innerHTML='<div class="empty">Nenhum relat\xF3rio salvo.</div>';return}reports.innerHTML='<table><thead><tr><th>A\xE7\xF5es</th><th>Data</th><th>Igreja</th><th>TOTVS</th><th>Respons\xE1vel</th><th>Membros</th><th>Entradas</th><th>Sa\xEDdas</th><th>Saldo</th><th>D\xEDzimo</th><th>Transferido</th><th>Documentos</th><th>Relat\xF3rio</th></tr></thead><tbody>'+data.map(r=>'<tr><td><button class="table-action" type="button" data-edit-report="'+escapeHtml(r.id)+'">Editar</button><button class="table-action danger" type="button" data-delete-report="'+escapeHtml(r.id)+'">Excluir</button></td><td>'+escapeHtml(r.report_date)+'</td><td>'+escapeHtml(r.church)+'</td><td>'+escapeHtml(r.totvs_code||'\u2014')+'</td><td>'+escapeHtml(r.responsible)+'</td><td>'+r.members+'</td><td class="amount">R$ '+currency(r.income)+'</td><td class="amount">R$ '+currency(r.expenses)+'</td><td class="amount '+(r.balance<0?'negative':'positive')+'">R$ '+currency(r.balance)+'</td><td class="amount">R$ '+currency(r.tithe||0)+'</td><td class="amount">R$ '+currency(r.transferred||0)+'</td><td>'+(r.documents.length?r.documents.map(d=>'<a href="/financeiro/documentos/'+encodeURIComponent(d.id)+'" target="_blank">'+escapeHtml(d.filename)+'</a>').join('<br>'):'\u2014')+'</td><td><a class="button-link" href="/financeiro/relatorios/'+encodeURIComponent(r.id)+'/imprimir" target="_blank">Exportar PDF</a></td></tr>').join('')+'</tbody></table>'}catch(error){reports.innerHTML='<div class="empty">'+escapeHtml(error.message)+'</div>'}}
...
function dashboardCsv(data) {
  ...
  const headers = ["M\xEAs de refer\xEAncia", "Igreja", "TOTVS", "Classifica\xE7\xE3o", "Cidade", "Estado", "Situa\xE7\xE3o", "Entradas", "Sa\xEDdas", "Saldo", "D\xEDzimo informativo", "Transferido", "\xDAltimo envio"];
